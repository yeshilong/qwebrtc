...
class CVideoEncoder : public VideoEncoder
{
  public:
    ...
    int32_t RegisterEncodeCompleteCallback(EncodedImageCallback *callback) override
    {
        if (callback)
        {
            encoder_->setCallback([callback](RTCEncodedImage *frame, IRTCCodecSpecificInfo *info) {
                ...
                {
                    codecSpecificInfo = static_cast<RTCCodecSpecificInfoH264 *>(info)
                                            ->d_ptr->nativeCodecSpecificInfo();
                }
    ...
    {
        return encoder_->releaseEncoder();
    }
...
{
  public:
    CVideoEncoderSelector(IRTCVideoEncoderSelector *selector)
    {
        selector_ = selector;
    }
    ...
    {
        RTCVideoCodecInfo *info = selector_->encoderForResolutionChangeBySize(
...
std::unique_ptr<VideoEncoder> CVideoEncoderFactory::CreateVideoEncoder(const SdpVideoFormat &format)
{
    RTCVideoCodecInfoPrivate *privateInfo = new RTCVideoCodecInfoPrivate(format);
    RTCVideoCodecInfo *info = new RTCVideoCodecInfo(*privateInfo);
...
std::unique_ptr<webrtc::VideoEncoderFactory::EncoderSelectorInterface> CVideoEncoderFactory::
