...
void RTCMediaStreamPrivate::removeAudioTrack(RTCAudioTrack *audioTrack)
{
    ...
    if (nativeMediaStream_->RemoveTrack(
            ((RTCAudioTrackPrivate *)(audioTrack->d_ptr))->nativeAudioTrack()))
    {
        audioTracks_.erase(std::ranges::remove(audioTracks_, audioTrack).begin(),
                           audioTracks_.end());
    }
}
...
rtc::scoped_refptr<webrtc::MediaStreamInterface> RTCMediaStreamPrivate::nativeMediaStream() const
{
    return nativeMediaStream_;
}
